...
var require_buffer = __commonJS({
  "node_modules/buffer/index.js"(exports) {
    ...
    function from(value3, encodingOrOffset, length) {
      ...
      if (value3 == null) {
        throw new TypeError("The first argument must be one of type string, Buffer, ArrayBuffer, Array, or Array-like Object. Received type " + typeof value3);
      }
      ...
      if (typeof Symbol !== "undefined" && Symbol.toPrimitive != null && typeof value3[Symbol.toPrimitive] === "function") {
        return Buffer.from(value3[Symbol.toPrimitive]("string"), encodingOrOffset, length);
      }
      throw new TypeError("The first argument must be one of type string, Buffer, ArrayBuffer, Array, or Array-like Object. Received type " + typeof value3);
    }
...
function setBigUint64(view, byteOffset, value3, isLE2) {
  if (typeof view.setBigUint64 === "function") return view.setBigUint64(byteOffset, value3, isLE2);
...
function abool(title, value3) {
  if (typeof value3 !== "boolean") throw new Error(title + " boolean expected, got " + value3);
}
...
function tonelliShanks(P) {
  ...
  if (S === 1) {
    ...
    return function tonelliFast(Fp4, n) {
      ...
      if (!Fp4.eql(Fp4.sqr(root), n)) throw new Error("Cannot find square root");
      return root;
    };
  }
  const Q1div2 = (Q + _1n2) / _2n2;
  return function tonelliSlow(Fp4, n) {
    if (Fp4.pow(n, legendreC) === Fp4.neg(Fp4.ONE)) throw new Error("Cannot find square root");
    let r = S;
    let g = Fp4.pow(Fp4.mul(Fp4.ONE, Z), Q);
    let x = Fp4.pow(n, Q1div2);
    let b = Fp4.pow(n, Q);
    while (!Fp4.eql(b, Fp4.ONE)) {
      if (Fp4.eql(b, Fp4.ZERO)) return Fp4.ZERO;
      let m = 1;
      for (let t2 = Fp4.sqr(b); m < r; m++) {
        if (Fp4.eql(t2, Fp4.ONE)) break;
        t2 = Fp4.sqr(t2);
      }
...
function FpSqrtEven(Fp4, elm) {
  if (!Fp4.isOdd) throw new Error("Field doesn't have isOdd");
...
function i2osp(value3, length) {
  anum(value3);
  anum(length);
  if (value3 < 0 || value3 >= 1 << 8 * length) throw new Error("invalid I2OSP input: " + value3);
...
function weierstrassPoints(opts) {
  ...
  if (!Fp4.eql(Fp4.sqr(CURVE.Gy), weierstrassEquation(CURVE.Gx))) throw new Error("bad generator point: equation left != right");
  ...
  const toAffineMemo = memoized((p, iz) => {
    ...
    if (Fp4.eql(z, Fp4.ONE)) return {
      x,
      y
    };
    const is0 = p.is0();
    if (iz == null) iz = is0 ? Fp4.ONE : Fp4.inv(z);
    ...
    if (!Fp4.eql(zz, Fp4.ONE)) throw new Error("invZ was invalid");
    return {
      x: ax,
      y: ay
    };
  ...
  const assertValidMemo = memoized((p) => {
    if (p.is0()) {
      if (CURVE.allowInfinityPoint && !Fp4.is0(p.py)) return;
    ...
    if (!Fp4.isValid(x) || !Fp4.isValid(y)) throw new Error("bad point: x or y not FE");
    const left = Fp4.sqr(y);
    const right = weierstrassEquation(x);
    if (!Fp4.eql(left, right)) throw new Error("bad point: equation left != right");
  ...
  class Point {
    constructor(px, py, pz) {
      ...
      this.py = py;
      this.pz = pz;
      if (px == null || !Fp4.isValid(px)) throw new Error("x required");
      if (py == null || !Fp4.isValid(py)) throw new Error("y required");
      if (pz == null || !Fp4.isValid(pz)) throw new Error("z required");
      Object.freeze(this);
    }
    ...
    static fromAffine(p) {
      ...
      if (!p || !Fp4.isValid(x) || !Fp4.isValid(y)) throw new Error("invalid affine point");
    ...
    hasEvenY() {
      ...
      if (Fp4.isOdd) return !Fp4.isOdd(y);
...
function mapToCurveSimpleSWU(Fp4, opts) {
  validateField(Fp4);
  if (!Fp4.isValid(opts.A) || !Fp4.isValid(opts.B) || !Fp4.isValid(opts.Z)) throw new Error("mapToCurveSimpleSWU: invalid opts");
  const sqrtRatio = SWUFpSqrtRatio(Fp4, opts.Z);
  if (!Fp4.isOdd) throw new Error("Fp.isOdd is not implemented!");
  return (u) => {
...
function bls(CURVE) {
  ...
  const G1_ = weierstrassPoints(__spreadValues({
    n: Fr2.ORDER
  }, CURVE.G1));
  const G1 = Object.assign(G1_, createHasher(G1_.ProjectivePoint, CURVE.G1.mapToCurve, __spreadValues(__spreadValues({}, CURVE.htfDefaults), CURVE.G1.htfDefaults)));
  const G2_ = weierstrassPoints(__spreadValues({
    n: Fr2.ORDER
  }, CURVE.G2));
  const G2 = Object.assign(G2_, createHasher(G2_.ProjectivePoint, CURVE.G2.mapToCurve, __spreadValues(__spreadValues({}, CURVE.htfDefaults), CURVE.G2.htfDefaults)));
  let lineFunction;
  ...
  function millerLoopBatch(pairs, withFinalExponent = false) {
    ...
    if (BLS_X_IS_NEGATIVE) f12 = Fp122.conjugate(f12);
...
function psiFrobenius(Fp4, Fp22, base) {
  ...
  if (!Fp22.eql(PSI2_Y, Fp22.neg(Fp22.ONE))) throw new Error("psiFrobenius: PSI2_Y!==-1");
  function psi2(x, y) {
...
function tower12(opts) {
  ...
  const Fp22 = {
    ...
    inv: ({
      ...
      return {
        c0: Fp4.mul(factor, Fp4.create(a)),
        c1: Fp4.mul(factor, Fp4.create(-b))
      };
    },
    sqrt: (num) => {
      ...
      if (Fp4.is0(c1)) {
        if (Fp4.eql(FpLegendre2(Fp4, c0), Fp4.ONE)) return Fp22.create({
          c0: Fp4.sqrt(c0),
          c1: Fp4.ZERO
        });
        else return Fp22.create({
          c0: Fp4.ZERO,
          c1: Fp4.sqrt(Fp4.div(c0, FpNONRESIDUE))
        });
      }
      ...
      if (!Fp4.is0(legendre) && !Fp4.eql(legendre, Fp4.ONE)) d = Fp4.sub(d, a);
      const a0 = Fp4.sqrt(d);
      const candidateSqrt = Fp22.create({
        c0: a0,
        c1: Fp4.div(Fp4.mul(c1, Fpdiv2), a0)
      });
      if (!Fp22.eql(Fp22.sqr(candidateSqrt), num)) throw new Error("Cannot find square root");
    ...
    // Bytes util
    fromBytes(b) {
      if (b.length !== Fp22.BYTES) throw new Error("fromBytes invalid length=" + b.length);
      return {
        c0: Fp4.fromBytes(b.subarray(0, Fp4.BYTES)),
        c1: Fp4.fromBytes(b.subarray(Fp4.BYTES))
      };
    },
    toBytes: ({
      c0,
      c1
    }) => concatBytes2(Fp4.toBytes(c0), Fp4.toBytes(c1)),
    cmov: ({
      c0,
      c1
    }, {
  ...
  const Fp62 = {
    ...
    is0: ({
      ...
      c1,
      c2
    }) => Fp22.is0(c0) && Fp22.is0(c1) && Fp22.is0(c2),
    neg: ({
      c0,
      c1,
      c2
    ...
    // Bytes utils
    fromBytes: (b) => {
      if (b.length !== Fp62.BYTES) throw new Error("fromBytes invalid length=" + b.length);
      return {
    ...
    toBytes: ({
      ...
      c1,
      c2
    }) => concatBytes2(Fp22.toBytes(c0), Fp22.toBytes(c1), Fp22.toBytes(c2)),
    cmov: ({
      c0,
      c1,
      c2
  ...
  const Fp122 = {
    ...
    isValid: ({
      c0,
      c1
    }) => Fp62.isValid(c0) && Fp62.isValid(c1),
    is0: ({
      c0,
      c1
    }) => Fp62.is0(c0) && Fp62.is0(c1),
    neg: ({
      c0,
      c1
    }) => ({
    ...
    // Bytes utils
    fromBytes: (b) => {
      if (b.length !== Fp122.BYTES) throw new Error("fromBytes invalid length=" + b.length);
      return {
    ...
    toBytes: ({
      c0,
      c1
    }) => concatBytes2(Fp62.toBytes(c0), Fp62.toBytes(c1)),
    cmov: ({
      c0,
      c1
    }, {
    ...
    frobeniusMap(lhs, power) {
      const {
        ...
        c1,
        c2
      } = Fp62.frobeniusMap(lhs.c1, power);
...
var {
  G2psi,
  G2psi2
} = psiFrobenius(Fp, Fp2, Fp2.div(Fp2.ONE, Fp2.NONRESIDUE));
...
var COMPRESSED_ZERO = setMask(Fp.toBytes(_0n7), {
  infinity: true,
  compressed: true
});
...
function signatureG2ToRawBytes(point) {
  point.assertValidity();
  const len = Fp.BYTES;
  if (point.equals(bls12_381.G2.ProjectivePoint.ZERO)) return concatBytes2(COMPRESSED_ZERO, numberToBytesBE(_0n7, len));
  const {
    x,
    y
  } = point.toAffine();
  ...
  const z2 = x0;
  return concatBytes2(setMask(numberToBytesBE(x1, len), {
    sort,
    compressed: true
...
var bls12_381 = bls({
  ...
  G1: {
    ...
    a: Fp.ZERO,
    b: _4n3,
    htfDefaults: __spreadProps(__spreadValues({}, htfDefaults), {
      m: 1,
    ...
    fromBytes: (bytes) => {
      ...
      if (value3.length === 48 && compressed) {
        ...
        if (infinity) {
          if (x !== _0n7) throw new Error("G1: non-empty compressed point at infinity");
          return {
            x: _0n7,
            y: _0n7
          };
        ...
        if (y * _2n6 / P !== BigInt(sort)) y = Fp.neg(y);
        return {
          x: Fp.create(x),
          y: Fp.create(y)
        };
        ...
        if (infinity) {
          if (x !== _0n7 || y !== _0n7) throw new Error("G1: non-empty point at infinity");
  ...
  G2: {
    ...
    toBytes: (c, point, isCompressed) => {
      ...
      if (isCompressed) {
        if (isZero) return concatBytes2(COMPRESSED_ZERO, numberToBytesBE(_0n7, len));
        const flag = Boolean(y.c1 === _0n7 ? y.c0 * _2n6 / P : y.c1 * _2n6 / P);
        return concatBytes2(setMask(numberToBytesBE(x.c1, len), {
          compressed: true,
          sort: flag
        }), numberToBytesBE(x.c0, len));
      } else {
        if (isZero) return concatBytes2(new Uint8Array([64]), new Uint8Array(4 * len - 1));
        const {
          re: x0,
          im: x1
        } = Fp2.reim(x);
...
var SignIdentity = class {
  ...
  transformRequest(request2) {
    return __async(this, null, function* () {
      const {
        body
      } = request2, fields = __rest(request2, ["body"]);
      const requestId = requestIdOf(body);
      return Object.assign(Object.assign({}, fields), {
        body: {
          content: body,
          sender_pubkey: this.getPublicKey().toDer(),
          sender_sig: yield this.sign(concat(domainSeparator, requestId))
        }
      });
    });
  }
...
var AnonymousIdentity = class {
  ...
  transformRequest(request2) {
    return __async(this, null, function* () {
      return Object.assign(Object.assign({}, request2), {
        body: {
          content: request2.body
        }
      });
    });
  }
...
function makeNonceTransform(nonceFn = makeNonce) {
  return (request2) => __async(this, null, function* () {
...
function twistedEdwards(curveDef) {
  ...
  const toAffineMemo = memoized((p, iz) => {
    ...
    const is0 = p.is0();
    if (iz == null) iz = is0 ? _8n2 : Fp4.inv(z);
    const ax = modP(x * iz);
    const ay = modP(y * iz);
    const zz = modP(z * iz);
    if (is0) return {
    ...
    if (zz !== _1n8) throw new Error("invZ was invalid");
    return {
      x: ax,
      y: ay
    };
  ...
  class Point {
    ...
    multiplyUnsafe(scalar, acc = Point.ZERO) {
      ...
      if (n === _0n8) return I;
      if (this.is0() || n === _1n8) return this;
...
function montgomery(curveDef) {
  ...
  function scalarMult(scalar, u) {
    ...
    if (pu === _0n9) throw new Error("invalid private or public key received");
...
var RistPoint = class _RistPoint {
  ...
  static fromHex(hex) {
    ...
    const y = mod2(u1 * Dy);
    const t = mod2(x * y);
    if (!isValid || isNegativeLE(t, P) || y === _0n10) throw new Error(emsg);
...
var ExpirableMap = class {
  ...
  constructor(options = {}) {
    ...
    const {
      source = [],
      expirationTime = 10 * 60 * 1e3
    } = options;
    const currentTime = Date.now();
    __classPrivateFieldSet(this, _ExpirableMap_inner, new Map([...source].map(([key, value3]) => [key, {
      value: value3,
...
var HttpAgent = class _HttpAgent {
  ...
  addTransform(type, fn, priority = fn.priority || 0) {
    if (type === "update") {
      ...
      __classPrivateFieldGet4(this, _HttpAgent_updatePipeline, "f").splice(i >= 0 ? i : __classPrivateFieldGet4(this, _HttpAgent_updatePipeline, "f").length, 0, Object.assign(fn, {
        priority
      }));
    } else if (type === "query") {
      const i = __classPrivateFieldGet4(this, _HttpAgent_queryPipeline, "f").findIndex((x) => (x.priority || 0) < priority);
      __classPrivateFieldGet4(this, _HttpAgent_queryPipeline, "f").splice(i >= 0 ? i : __classPrivateFieldGet4(this, _HttpAgent_queryPipeline, "f").length, 0, Object.assign(fn, {
        priority
      }));
    }
  }
  getPrincipal() {
    return __async(this, null, function* () {
      ...
      return (yield __classPrivateFieldGet4(this, _HttpAgent_identity, "f")).getPrincipal();
    });
  }
  call(canisterId, options, identity) {
    return __async(this, null, function* () {
      var _a2, _b2;
      yield __classPrivateFieldGet4(this, _HttpAgent_instances, "m", _HttpAgent_rootKeyGuard).call(this);
      const callSync = (_a2 = options.callSync) !== null && _a2 !== void 0 ? _a2 : true;
      const id = yield identity !== void 0 ? yield identity : yield __classPrivateFieldGet4(this, _HttpAgent_identity, "f");
      if (!id) {
        throw new IdentityInvalidError("This identity has expired due this application's security policy. Please refresh your authentication.");
      }
      ...
      let transformedRequest = yield this._transform({
        request: {
          ...
          }, __classPrivateFieldGet4(this, _HttpAgent_credentials, "f") ? {
      ...
      try {
        const requestSync = () => {
          ...
          return __classPrivateFieldGet4(this, _HttpAgent_fetch, "f").call(this, "" + new URL(`/api/v3/canister/${ecid.toText()}/call`, this.host), Object.assign(Object.assign(Object.assign({}, __classPrivateFieldGet4(this, _HttpAgent_callOptions, "f")), transformedRequest.request), {
            body
          }));
        };
        const requestAsync = () => {
          this.log.print(`fetching "/api/v2/canister/${ecid.toText()}/call" with request:`, transformedRequest);
          return __classPrivateFieldGet4(this, _HttpAgent_fetch, "f").call(this, "" + new URL(`/api/v2/canister/${ecid.toText()}/call`, this.host), Object.assign(Object.assign(Object.assign({}, __classPrivateFieldGet4(this, _HttpAgent_callOptions, "f")), transformedRequest.request), {
            body
          }));
        };
        ...
        if (responseBody && "certificate" in responseBody) {
          const time = yield this.parseTimeFromResponse({
  ...
  query(canisterId, fields, identity) {
    return __async(this, null, function* () {
      ...
      let queryResult;
      const id = yield identity !== void 0 ? identity : __classPrivateFieldGet4(this, _HttpAgent_identity, "f");
      if (!id) {
        throw new IdentityInvalidError("This identity has expired due this application's security policy. Please refresh your authentication.");
      }
      const canister = Principal.from(canisterId);
      ...
      const makeQuery = () => __async(this, null, function* () {
        return {
          ...
          query: yield __classPrivateFieldGet4(this, _HttpAgent_instances, "m", _HttpAgent_requestAndRetryQuery).call(this, args)
        };
      });
      ...
      try {
        const [_queryResult, subnetStatus] = yield Promise.all([makeQuery(), getSubnetStatus()]);
  ...
  readState(canisterId, fields, identity, request2) {
    return __async(this, null, function* () {
      var _a2, _b2, _c, _d;
      ...
      const transformedRequest = request2 !== null && request2 !== void 0 ? request2 : yield this.createReadStateRequest(fields, identity);
      const body = encode2(transformedRequest.body);
      this.log.print(`fetching "/api/v2/canister/${canister}/read_state" with request:`, transformedRequest);
      const backoff = __classPrivateFieldGet4(this, _HttpAgent_backoffStrategy, "f").call(this);
      try {
        const response = yield __classPrivateFieldGet4(this, _HttpAgent_instances, "m", _HttpAgent_requestAndRetry).call(this, {
          request: () => __classPrivateFieldGet4(this, _HttpAgent_fetch, "f").call(this, "" + new URL(`/api/v2/canister/${canister.toString()}/read_state`, this.host), Object.assign(Object.assign(Object.assign({}, __classPrivateFieldGet4(this, _HttpAgent_fetchOptions, "f")), transformedRequest.request), {
            body
          })),
          backoff,
          tries: 0
...
`);
        ...
        const decodedResponse = decode2(yield response.arrayBuffer());
        this.log.print("Read state response:", decodedResponse);
        const parsedTime = yield this.parseTimeFromResponse(decodedResponse);
        if (parsedTime > 0) {
  ...
  syncTime(canisterId) {
    return __async(this, null, function* () {
      yield __classPrivateFieldGet4(this, _HttpAgent_instances, "m", _HttpAgent_rootKeyGuard).call(this);
      const CanisterStatus = yield import("./canisterStatus-6NREMNJL.js");
      ...
      try {
        ...
        const status = yield CanisterStatus.request({
  ...
  status() {
    return __async(this, null, function* () {
      ...
      const response = yield __classPrivateFieldGet4(this, _HttpAgent_instances, "m", _HttpAgent_requestAndRetry).call(this, {
        backoff,
        request: () => __classPrivateFieldGet4(this, _HttpAgent_fetch, "f").call(this, "" + new URL(`/api/v2/status`, this.host), Object.assign({
          headers
        }, __classPrivateFieldGet4(this, _HttpAgent_fetchOptions, "f"))),
        tries: 0
      });
      return decode2(yield response.arrayBuffer());
    });
  }
  fetchRootKey() {
    return __async(this, null, function* () {
      ...
      if (__classPrivateFieldGet4(this, _HttpAgent_rootKeyPromise, "f")) {
        result = yield __classPrivateFieldGet4(this, _HttpAgent_rootKeyPromise, "f");
      } else {
...
_HttpAgent_rootKeyPromise = /* @__PURE__ */ new WeakMap(), _HttpAgent_shouldFetchRootKey = /* @__PURE__ */ new WeakMap(), _HttpAgent_identity = /* @__PURE__ */ new WeakMap(), _HttpAgent_fetch = /* @__PURE__ */ new WeakMap(), _HttpAgent_fetchOptions = /* @__PURE__ */ new WeakMap(), _HttpAgent_callOptions = /* @__PURE__ */ new WeakMap(), _HttpAgent_timeDiffMsecs = /* @__PURE__ */ new WeakMap(), _HttpAgent_credentials = /* @__PURE__ */ new WeakMap(), _HttpAgent_retryTimes = /* @__PURE__ */ new WeakMap(), _HttpAgent_backoffStrategy = /* @__PURE__ */ new WeakMap(), _HttpAgent_maxIngressExpiryInMinutes = /* @__PURE__ */ new WeakMap(), _HttpAgent_waterMark = /* @__PURE__ */ new WeakMap(), _HttpAgent_queryPipeline = /* @__PURE__ */ new WeakMap(), _HttpAgent_updatePipeline = /* @__PURE__ */ new WeakMap(), _HttpAgent_subnetKeys = /* @__PURE__ */ new WeakMap(), _HttpAgent_verifyQuerySignatures = /* @__PURE__ */ new WeakMap(), _HttpAgent_verifyQueryResponse = /* @__PURE__ */ new WeakMap(), _HttpAgent_instances = /* @__PURE__ */ new WeakSet(), _HttpAgent_requestAndRetryQuery = function _HttpAgent_requestAndRetryQuery2(args) {
  return __async(this, null, function* () {
    ...
    try {
      ...
      const fetchResponse = yield __classPrivateFieldGet4(this, _HttpAgent_fetch, "f").call(this, "" + new URL(`/api/v2/canister/${ecid.toString()}/query`, this.host), Object.assign(Object.assign(Object.assign({}, __classPrivateFieldGet4(this, _HttpAgent_fetchOptions, "f")), transformedRequest.request), {
        body
      }));
      if (fetchResponse.status === 200) {
        const queryResponse = decode2(yield fetchResponse.arrayBuffer());
...
`, {
  ...
  ${error}
  Retrying query.`);
        return yield __classPrivateFieldGet4(this, _HttpAgent_instances, "m", _HttpAgent_requestAndRetryQuery2).call(this, Object.assign(Object.assign({}, args), {
          tries: tries + 1
        }));
      }
      throw error;
...
}, _HttpAgent_requestAndRetry = function _HttpAgent_requestAndRetry2(args) {
  return __async(this, null, function* () {
    const {
      request: request2,
      backoff,
    ...
    let response;
    try {
      response = yield request2();
    } catch (error) {
      if (__classPrivateFieldGet4(this, _HttpAgent_retryTimes, "f") > tries) {
        this.log.warn(`Caught exception while attempting to make request:
  ${error}
  Retrying request.`);
        return yield __classPrivateFieldGet4(this, _HttpAgent_instances, "m", _HttpAgent_requestAndRetry2).call(this, {
          request: request2,
          backoff,
          tries: tries + 1
        });
    ...
    const responseText = yield response.clone().text();
...
}, _HttpAgent_rootKeyGuard = function _HttpAgent_rootKeyGuard2() {
  return __async(this, null, function* () {
    if (this.rootKey) {
      return;
...
var ProxyAgent = class {
  ...
  fetchRootKey() {
    return __async(this, null, function* () {
      const rootKey = (yield this.status()).root_key;
      this.rootKey = rootKey;
      return rootKey;
    });
  }
...
var Certificate = class _Certificate {
  ...
  _checkDelegationAndGetKey(d) {
    return __async(this, null, function* () {
      ...
      const cert = yield _Certificate.createUnverified({
...
function reconstruct(t) {
  return __async(this, null, function* () {
    switch (t[0]) {
      ...
      case NodeType.Labeled:
        return hash(concat(domain_sep("ic-hashtree-labeled"), t[1], yield reconstruct(t[2])));
      case NodeType.Fork:
        return hash(concat(domain_sep("ic-hashtree-fork"), yield reconstruct(t[1]), yield reconstruct(t[2])));
      default:
...
var request = (options) => __async(void 0, null, function* () {
  const {
    agent,
    paths
  } = options;
...
@noble/curves/esm/ed25519.js:
  (*! noble-curves - MIT License (c) 2022 Paul Miller (paulmillr.com) *)
*/
//# sourceMappingURL=chunk-HHVHVY75.js.map
